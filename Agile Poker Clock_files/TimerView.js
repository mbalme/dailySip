...
function mergerequest(author,name){
	var settings = {
	  "url": "https://gitlab.canal-plus.com/api/v4/groups/104/merge_requests?private_token=LdGEg-rnZ3dSVGV5JAAs&state=opened",
	  "method": "GET",
	  "timeout": 0,
	  "headers": {
		"Private-Token": "true",
	...
	$.ajax(settings).done(function (response) {
	  ...
	  //console.log(response);
		...
		if(item2["author"]["username"]==author){
					 // console.log(item2);
			//console.log(item2["title"]);
			var intervalTime = new Date(Date.now()-Date.parse(item2["created_at"]));
			var strDuration=parseInt(intervalTime/86400/1000)+'d '+intervalTime.toUTCString().replace(/.*(\d{2}):(\d{2}):(\d{2}).*/, "$1h")
			rett+=rett+"<br/>"+item2["references"]["relative"].toString().replace(item2["references"]["short"],"")+" | "+item2["title"].toString() + " | <i>"+strDuration+"</i>";  
			//console.log(rett);
		  }
